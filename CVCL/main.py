import argparse  # 导入argparse库用于命令行参数解析
import warnings  # 导入warnings库用于控制警告

from models import *  # 导入模型定义
from layers import *  # 导入自定义层
from loss import *  # 导入损失函数

import torch  # 导入PyTorch库
import scipy.io as sio  # 导入SciPy库用于MAT文件操作
...
if __name__ == "__main__":
    ...
    mv_data = MultiviewData(args.db, device)  # 加载多视图数据
    num_views = len(mv_data.data_views)  # 获取视图数量
    num_samples = mv_data.labels.size  # 获取样本数量
    num_clusters = np.unique(mv_data.labels).size  # 获取聚类数量

    input_sizes = np.zeros(num_views, dtype=int)  # 初始化输入尺寸数组
    for idx in range(num_views):
        input_sizes[idx] = mv_data.data_views[idx].shape[1]  # 获取每个视图的特征维度
    ...
    mvc_loss = DeepMVCLoss(args.batch_size, num_clusters)  # 初始化损失函数
    optimizer = torch.optim.Adam(mnw.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)  # 初始化优化器

    if args.load_model:  # 如果加载模型
        state_dict = torch.load('./models/CVCL_pytorch_model_%s.pth' % args.db)  # 加载模型参数
        mnw.load_state_dict(state_dict)  # 将参数加载到模型中

    else:  # 否则进行训练
        pre_train_loss_values = pre_train(mnw, mv_data, args.batch_size, args.mse_epochs, optimizer)  # 预训练

        # sio.savemat('pre_train_loss_%s.mat' % args.db, {'data': pre_train_loss_values})  # 保存预训练损失

        t = time.time()  # 记录时间
        fine_tuning_loss_values = np.zeros(args.con_epochs, dtype=np.float64)  # 初始化微调损失数组
        for epoch in range(args.con_epochs):  # 进行微调
        ...
        # sio.savemat('fine_tuning_loss_%s.mat' % args.db, {'data': fine_tuning_loss_values})  # 保存微调损失

        print("contrastive_train finished.")  # 打印训练完成消息
        print("Total time elapsed: {:.2f}s".format(time.time() - t))  # 打印总耗时

        if args.save_model:  # 如果需要保存模型
            torch.save(mnw.state_dict(), './models/CVCL_pytorch_model_%s.pth' % args.db)  # 保存模型参数

    acc, nmi, pur, ari = valid(mnw, mv_data, args.batch_size)  # 验证模型性能
    # 将结果写入文件
    with open('result_%s.txt' % args.db, 'a+') as f:
        f.write('{} \t {} \t {} \t {} \t {} \t {} \t {} \t {:.6f} \t {:.6f} \t {:.6f} \t {:.4f} \n'.format(
            dim_high_feature, dim_low_feature, args.seed, args.batch_size,
